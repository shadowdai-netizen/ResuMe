...
const RESUME_DATA_JSON_SCHEMA = {
  ...
  properties: {
    ...
    basicInfo: {
      type: 'object',
      required: [
        'name', 'phone', 'email', 'age', 'gender', 'maritalStatus', 'currentStatus', 'targetCity',
        'expectedPosition', 'salaryMin', 'salaryMax', 'personalWebsite', 'wechat', 'linkedin',
        'height', 'weight', 'ethnicity', 'nativePlace', 'zodiac', 'mbti', 'politicalStatus',
      ],
      properties: {
        name: { type: 'string' },
        phone: { type: 'string' },
...
export function createResumeSkillMarkdown(data: ResumeData): string {
  ...
  return [
    ...
    '- `basicInfo` 可选信息包括期望职位、薪资范围、社交账号、身高体重、民族、籍贯、星座、MBTI 和政治面貌。',
    '- `education`: 教育经历数组。',
...
function normalizeResumeData(source: Record<string, unknown>): ResumeData {
  ...
  const normalized: ResumeData = {
    ...
    basicInfo: {
      ...
      expectedPosition: asString(basicInfoSource.expectedPosition),
      salaryMin: asString(basicInfoSource.salaryMin),
      salaryMax: asString(basicInfoSource.salaryMax),
      personalWebsite: asString(basicInfoSource.personalWebsite),
      wechat: asString(basicInfoSource.wechat),
      linkedin: asString(basicInfoSource.linkedin),
      height: asString(basicInfoSource.height),
      weight: asString(basicInfoSource.weight),
      ethnicity: asString(basicInfoSource.ethnicity),
      nativePlace: asString(basicInfoSource.nativePlace),
      zodiac: asString(basicInfoSource.zodiac),
      mbti: asString(basicInfoSource.mbti),
      politicalStatus: asString(basicInfoSource.politicalStatus),
    },
